        ...
        <div>
            <div class="grid grid-flow-row grid-cols-2 grid-rows-auto">
                @foreach ($project->images as $image)
                    <div style="position: relative;" class="min-h-[140px] py-2">
                        <form
                            action="{{ route('projects.images.destroy', ['no' => $project->no, 'imageNo' => $image->id]) }}"
                            method="POST">
                            @csrf
                            @method('DELETE')
                        ...
                        <img src="{{ asset($image->name) }}" alt="Image 1" width="140" class="m-auto">
                    </div>
                @endforeach

                @if ($project->images->count() < 6)
                    <a href="{{ route('projects.images.create', ['no' => $project->no]) }}"
                        class="w-28 h-28 mt-10 m-auto text-center bg-slate-400 rounded-full text-gray-100">
